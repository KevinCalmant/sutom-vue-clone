...
export default {
  ...
  beforeCreate() {
    for (let tryIndex of Array(6).keys()) {
      this.word.split("").forEach((letter, index) => {
        this.gameStore.$patch((state) => {
          if (!state.guesses[tryIndex]) {
            state.guesses[tryIndex] = [];
          }
          if (tryIndex === 0) {
...
#grille {
  ...
  min-height: calc(
    6 * var(--taille-cellule) + 12 * var(--epaisseur-bordure-cellule)
  );
}
